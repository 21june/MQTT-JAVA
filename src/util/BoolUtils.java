...
public class BoolUtils {
	...
	/**
	 * convert boolean array to byte. for example, booleans = {false, false, false, false, false, false, false, false} -> byte = 0
	 * 
	 * @param bools
	 *            boolean array
	 * @return byte
	...
	public static byte booleansToByte(boolean[] bools) {
		byte b = (byte) ((bools[0] ? 1 : 0) + (bools[1] ? 1 << 1 : 0) + (bools[2] ? 1 << 2 : 0) + (bools[3] ? 1 << 3 : 0) + (bools[4] ? 1 << 4 : 0) + (bools[5] ? 1 << 5 : 0) + (bools[6] ? 1 << 6 : 0)
				+ (bools[7] ? 1 << 7 : 0));
	...
	/**
	 * convert byte to booleans. for example, byte = 0 -> booleans = {false, false, false, false, false, false, false, false}
	 * 
	 * @param num
	 *            byte
	 * @return booleans
